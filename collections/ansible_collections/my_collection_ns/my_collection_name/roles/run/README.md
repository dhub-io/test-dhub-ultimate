My_collection_ns.My_collection_name Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: my_collection_ns.my_collection_name.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from my_collection_ns.my_collection_name
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: my_collection_ns.my_collection_name.run
      vars:
        run_x: 42
```
